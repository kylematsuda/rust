...
fn build_type_alias(cx: &mut DocContext<'_>, did: DefId) -> Box<clean::Typedef> {
    let predicates = cx.tcx.explicit_predicates_of(did);
    let type_ = clean_middle_ty(ty::Binder::dummy(cx.tcx.type_of(did).subst_identity()), cx, Some(did));
...
pub(crate) fn build_impl(
    ...
    let tcx = cx.tcx;
    let associated_trait = tcx.impl_trait_ref(did).map(|v| v.subst_identity());
    ...
    let for_ = match &impl_item {
        ...
        None => clean_middle_ty(ty::Binder::dummy(tcx.type_of(did).subst_identity()), cx, Some(did)),
    };
...
fn build_static(cx: &mut DocContext<'_>, did: DefId, mutable: bool) -> clean::Static {
    clean::Static {
        type_: clean_middle_ty(ty::Binder::dummy(cx.tcx.type_of(did).subst_identity()), cx, Some(did)),
