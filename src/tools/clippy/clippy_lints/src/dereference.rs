    ...
    let position = walk_to_expr_usage(cx, e, &mut |parent, child_id| {
        ...
        match parent {
            ...
            | Node::ImplItem(&ImplItem {
                ...
                let output = cx
                    .tcx
                    .erase_late_bound_regions(cx.tcx.fn_sig(owner_id.to_def_id()).subst_identity().output());
                ...
                ExprKind::MethodCall(method, receiver, args, _) => {
                    ...
                    if receiver.hir_id == child_id {
                        ...
                        if e.hir_id != child_id {
                            ...
                            && let impl_ty = if cx.tcx.fn_sig(id).skip_binder().skip_binder().inputs()[0].is_ref() {
                    ...
                    args.iter().position(|arg| arg.hir_id == child_id).map(|i| {
                        let ty = cx.tcx.fn_sig(id).skip_binder().skip_binder().inputs()[i + 1];
                        // `e.hir_id == child_id` for https://github.com/rust-lang/rust-clippy/issues/9739
    ...
    cx.tcx
        ...
        .any(|assoc_item| {
            if assoc_item.fn_has_self_parameter {
                let self_ty = cx.tcx.fn_sig(assoc_item.def_id).skip_binder().skip_binder().inputs()[0];
