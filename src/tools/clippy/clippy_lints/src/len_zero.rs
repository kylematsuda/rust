...
impl<'tcx> LateLintPass<'tcx> for LenZero {
    ...
    fn check_impl_item(&mut self, cx: &LateContext<'tcx>, item: &'tcx ImplItem<'_>) {
        if_chain! {
            ...
            if let Some(output) = parse_len_output(cx, cx.tcx.fn_sig(item.owner_id).skip_binder().skip_binder());
            then {
    ...
    let (msg, is_empty_span, self_kind) = match is_empty {
        ...
        Some(is_empty)
            if !(is_empty.fn_has_self_parameter
                && check_is_empty_sig(cx.tcx.fn_sig(is_empty.def_id).skip_binder().skip_binder(), self_kind, output)) =>
        {
            (
                format!(
