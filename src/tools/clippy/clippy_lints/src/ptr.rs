...
impl<'tcx> LateLintPass<'tcx> for Ptr {
    fn check_trait_item(&mut self, cx: &LateContext<'tcx>, item: &'tcx TraitItem<'_>) {
        if let TraitItemKind::Fn(sig, trait_method) = &item.kind {
            ...
            for arg in check_fn_args(
                cx,
                cx.tcx.fn_sig(item.owner_id).skip_binder().skip_binder().inputs(),
                sig.decl.inputs,
                &[],
            )
    ...
    fn check_body(&mut self, cx: &LateContext<'tcx>, body: &'tcx Body<'_>) {
        ...
        let decl = sig.decl;
        let sig = cx.tcx.fn_sig(item_id).skip_binder().skip_binder();
            ...
            match get_expr_use_or_unification_node(self.cx.tcx, e) {
                ...
                Some((Node::Expr(e), child_id)) => match e.kind {
                    ...
                    ExprKind::MethodCall(name, self_arg, expr_args, _) => {
                        ...
                        match *self.cx.tcx.fn_sig(id).skip_binder().skip_binder().inputs()[i].peel_refs().kind() {
